 ...
 * <p>
 * Thus, algorithm runs through each of d cycles and rotates complete array by k
 * positions to the right.
 * <p>
 * Time complexity is O(n). Memory complexity is O(1).
 * <p>
 * Of course, we need to find d = gcd(n, k). This is computed by Euclid algorithm
...
class RotateArray {
    ...
    public void rotate(int[] a, int k) {
        ...
        for (int i = 0; i < d; i += 1) {
            rotateCycle(a, k, i, len);
        }
    }

    private void rotateCycle(int[] a, int k, int i, int len) {
        int n = a.length;
        ...
        // math note: after len = n/d iterations algorithm return to the original index
    }

    // write val to a[i] and return old value
